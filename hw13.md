...
- **한 작품에만 도드라지는 단어**와 그것이 시사하는 작품의 특성: Frankenstein에서는 `father`이 도드라지는데, 이는 빅터 박사와 괴물의 **부자 관계** 속 갈등이 작품의 한 중심 축이 되고 있음을 시사한다. 이외에도 `life`, `death`를 통해 **생명 창조와 죽음**의 순환이 이루어지는 작품의 핵심 주제를 유추할 수 있다. 반면 Dracula에서는 `van`,`helsing`, `lucy`, `mina` 와 같은 고유명사가 도드라지는데, 이는 여러 인물의 일기로 구성되어 등장인물이 자주 호명되는 작품의 특성을 보여준다. 또한 `room`은 햇빛을 피해 실내에 머물 수밖에 없는 드라큘라의 공간적 특성을 반영한다. 
